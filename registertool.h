...
void timesearch(struct vip *vip_data, int vip_search_id)
{
    ...
    time(&currentTime);
    localTime = localtime(&currentTime);
    if ((vip_data[vip_search_id].last_consume_day.year < (localTime->tm_year+1900) && vip_data[vip_search_id].last_consume_day.month < localTime->tm_mon && vip_data[vip_search_id].last_consume_day.day < localTime->tm_mday)||(localTime->tm_year+1899)>vip_data[vip_search_id].last_consume_day.year)
    {
